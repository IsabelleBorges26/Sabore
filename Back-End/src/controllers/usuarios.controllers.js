...
const { getProfile, listUsers } = require("../data/restQueries");
const bcrypt = require("bcryptjs");
const jwt = require("jsonwebtoken");
const JWT_SECRET = process.env.JWT_SECRET || "sabore-secret-key-12345";
const defaultBooks = [
...
const perfil = async (req, res) => {
    const usuarioId = req.usuario.id;
    try {
        const usuario = await withDatabaseFallback(
            ...
            async () => {
                const profile = await getProfile(usuarioId);
                if (!profile) return null;

                return {
                    ...profile,
                    _count: {
                        receitas: profile.receitas.length,
                        favoritos: profile.favoritos.length,
                        livros: profile.livros.length
                    }
                };
            },
            "carregar perfil"
...
const listar = async (req, res) => {
    try {
        const lista = await withDatabaseFallback(
