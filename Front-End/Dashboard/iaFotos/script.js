    ...
    const stepInterval = setInterval(() => {
      ...
      if (currentStep < logs.length) {
        ...
        visionAnalysisPromise.then((data) => {
          ...
          const genericLabels = /alimento grelhado|legumes e verduras|acompanhamento base|refeição completa/i;
          if (!data.title || detections.length === 0 || detections.every(item => genericLabels.test(item.name || ''))) {
            throw new Error('A IA não identificou a comida com segurança. Tente uma foto mais nítida e aproximada.');
          }
          const foodData = {
            title: data.title || "Receita Sugerida",
            dishName: data.dishName || "Prato Identificado!",
            detections,
            // O serviço de visão não fornece coordenadas; não desenhamos caixas inventadas.
            boundingBoxes: [],
            recipe: null
          };
          ...
          document.getElementById('state-scanning').classList.add('hidden');
          document.getElementById('state-idle').classList.remove('hidden');
          alert(`Não foi possível analisar esta foto com IA: ${err.message || 'tente outra imagem em instantes.'}`);
        });
      }
    }, 700);
  }
      ...
      const recipeInterval = setInterval(() => {
        ...
        if (currentStep < recipeLogs.length) {
          ...
          recipePromise.then((recipeData) => {
            ...
            alert(`A análise foi concluída, mas a receita não pôde ser gerada: ${err.message || 'tente novamente.'}`);
          });
        }
      }, 650);
    });
